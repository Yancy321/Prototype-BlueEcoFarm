...
$recentEntries = $pdo->query("SELECT sr.id, p.name, sr.quantity, sr.warehouse_id, sr.transaction_date, sr.created_at FROM stock_records sr JOIN products p ON sr.product_id = p.id WHERE sr.record_type='incoming' AND sr.is_deleted = 0 AND DATE(sr.transaction_date) = CURDATE() ORDER BY sr.created_at DESC LIMIT 5")->fetchAll();
...
<style>
    ...
    .analytics-card .card-header { display:flex; justify-content:space-between; align-items:flex-start; gap:1rem; margin-bottom:1.25rem; }
    ...
    .analytics-card .button-secondary { background:#1b5e20; color:#fff; border:none; border-radius:999px; padding:0.8rem 1.25rem; cursor:pointer; font-weight:700; transition:background .2s ease; }
    .analytics-card .button-secondary:hover { background:#185d1e; }
    ...
    .entry-qty { font-weight:700; color:#1b5e20; }
...
<div class="analytics-card">
    <div class="card-header">
        <div>
            ...
            <p>Log new harvest or production into Farm inventory.</p>
        </div>
    </div>
    ...
    <div class="stock-grid">
        <div class="stock-form">
            <form id="stockInForm" class="form-layout">
        ...
        <aside class="recent-panel">
            ...
            <p>Latest incoming production records for the current day.</p>
