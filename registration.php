...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$fullName || !$username || !$email || !$password || !$confirmPass || !$businessName) {
        ...
        $usernameTaken = $chkUser->num_rows > 0;
        $chkUser->close();
        ...
        $emailTaken = $chkEmail->num_rows > 0;
        $chkEmail->close();
        ...
        if ($usernameTaken) {
            ...
            try {
                ...
                ");
                $stmtUser->bind_param("ssss", $username, $hashed, $fullName, $email);
                $stmtUser->execute();
...
<style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    body {
        font-family: 'DM Sans', sans-serif; background: #e8f0e6;
    ...
    .brand h1 { font-family: 'DM Serif Display', serif; font-size: 1.6rem; color: #2d5a27; font-weight: 400; }
    .brand p  { font-size: .82rem; color: #6b7c69; margin-top: 2px; }
    .section-label {
        font-size: .72rem; font-weight: 700; color: #9aab98;
        text-transform: uppercase; letter-spacing: .6px; margin: 20px 0 12px;
    }
    ...
    input[type="text"], input[type="email"], input[type="password"] {
        width: 100%; padding: 10px 13px; border: 1.5px solid #dde8db;
    ...
    .field-hint { font-size: .75rem; color: #9aab98; margin-top: 4px; }
    .pass-wrap { position: relative; }
    .pass-wrap input { padding-right: 40px; }
    .pass-toggle {
        position: absolute; right: 11px; top: 50%; transform: translateY(-50%);
        background: none; border: none; cursor: pointer; color: #9aab98;
        font-size: 1.05rem; padding: 0; line-height: 1; display: flex; align-items: center;
    }
    .pass-toggle:hover { color: #4a8c42; }
    .strength-bar { height: 4px; border-radius: 4px; background: #e2ece0; margin-top: 6px; overflow: hidden; }
    ...
    /* Success state */
    .success-wrap { display: flex; flex-direction: column; align-items: center; text-align: center; padding: 4px 0 8px; }
    .success-icon-ring {
        width: 76px; height: 76px; border-radius: 50%; background: #f0faf0;
        border: 2px solid #c4dfc0; display: flex; align-items: center; justify-content: center;
        margin-bottom: 20px; animation: popIn .4s cubic-bezier(.175,.885,.32,1.275) both;
    }
    .success-icon-ring i { font-size: 2.2rem; color: #2d5a27; }
    @keyframes popIn { from { transform: scale(.5); opacity: 0; } to { transform: scale(1); opacity: 1; } }
    .success-wrap h3 { font-family: 'DM Serif Display', serif; font-size: 1.35rem; color: #1a2e18; font-weight: 400; margin-bottom: 8px; }
    .success-wrap .sub { font-size: .85rem; color: #6b7c69; line-height: 1.7; margin-bottom: 16px; max-width: 340px; }
    .status-badge {
        display: inline-flex; align-items: center; gap: 6px;
        background: #fffbeb; color: #92400e; border: 1px solid #fde68a;
        border-radius: 20px; padding: 5px 14px; font-size: .75rem; font-weight: 700;
        letter-spacing: .3px; margin-bottom: 24px;
    }
    .steps-box { width: 100%; background: #f7fbf5; border: 1px solid #ddeedd; border-radius: 14px; padding: 18px 20px; text-align: left; margin-bottom: 24px; }
    .steps-heading { font-size: .7rem; font-weight: 700; text-transform: uppercase; letter-spacing: .7px; color: #9aab98; margin-bottom: 14px; }
    .step-row { display: flex; align-items: flex-start; gap: 13px; position: relative; padding-bottom: 14px; }
    .step-row:last-child { padding-bottom: 0; }
    .step-row:not(:last-child)::after { content: ''; position: absolute; left: 14px; top: 28px; bottom: 0; width: 1.5px; background: #ddeedd; }
    .step-node { width: 28px; height: 28px; border-radius: 50%; display: flex; align-items: center; justify-content: center; flex-shrink: 0; font-size: .8rem; font-weight: 700; position: relative; z-index: 1; }
    .step-node.done    { background: #d1f0cb; color: #2d5a27; }
    .step-node.pending { background: #e2ece0; color: #6b7c69; font-size: .78rem; }
    .step-content { padding-top: 3px; }
    .step-content strong { display: block; font-size: .85rem; color: #1a2e18; margin-bottom: 2px; }
    .step-content span   { font-size: .78rem; color: #6b7c69; line-height: 1.5; }
    .btn-back {
        display: inline-flex; align-items: center; gap: 7px; padding: 10px 26px;
        background: transparent; color: #2d5a27; border: 1.5px solid #4a8c42;
        border-radius: 10px; font-family: 'DM Sans', sans-serif;
        font-size: .88rem; font-weight: 600; text-decoration: none; transition: background .2s;
    }
    .btn-back:hover { background: #f0faf0; }
    .login-link { text-align: center; margin-top: 18px; font-size: .83rem; color: #6b7c69; }
...
<div class="card">
    ...
    <div class="success-wrap">
        <div class="success-icon-ring"><i class="bi bi-check-lg"></i></div>
        <h3>Application Submitted!</h3>
        <p class="sub">
            Thank you, <strong><?= htmlspecialchars($submittedName) ?></strong>.
            Your account is awaiting admin approval before you can log in.
        </p>
        <div class="status-badge"><i class="bi bi-hourglass-split"></i> Pending Approval</div>
        <div class="steps-box">
            <div class="steps-heading">What happens next</div>
            <div class="step-row">
                <div class="step-node done"><i class="bi bi-check"></i></div>
            ...
            <div class="step-row">
                ...
                <div class="step-content">
                    ...
                    <span>The Blue Eco Farm admin will review your application.
                    You can <a href="check_status.php" style="color:#2d5a27;font-weight:600;">check your status here</a> anytime.</span>
                </div>
            </div>
            <div class="step-row">
        ...
        <a href="login.php" class="btn-back"><i class="bi bi-arrow-left"></i> Back to Login</a>
    </div>
    ...
    <?php else: ?>
        ...
        <form method="POST" novalidate>
            ...
            <div class="form-group">
                ...
                <div class="field-hint">Used for password recovery only.</div>
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <div class="pass-wrap">
                        ...
                        <button type="button" class="pass-toggle" id="togglePwd"
                ...
                <div class="form-group">
                    ...
                    <div class="pass-wrap">
                        ...
                        <button type="button" class="pass-toggle" id="toggleCfm"
                                onclick="togglePass('confirmPassword','toggleCfm')">
                            <i class="bi bi-eye"></i>
                        </button>
                    </div>
                </div>
